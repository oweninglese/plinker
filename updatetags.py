...
def link_tags(file, tag):
    ...
    def add_tag(afile: str, tag: str) -> bool:
        ...
        if tag in post.content:
            ...
            with open(afile, "w", encoding="utf-8") as text:
    ...
    def enum_and_sub(testfile, abc, tag):
        ...
        with open(testfile, "r", encoding="utf-8") as file:
            text = file.readlines()
        with open(testfile, "w", encoding="utf-8") as source:
    ...
    if add_tag(file, tag) and tag not in CHECKED:
        ...
        enum_and_sub(file, checkforend(file), tag)
        CHECKED.add(tag)
...
def check_files(file, tag):
    ...
    """
    # print('Entered check_files')
    if file.endswith(".md"):
        # print('checked file for md')
        link_tags(file, tag)
...
ARTS = "/home/ofoo/Dev/md_project/arts/arts_copy"
...
def run(file, tags):
    """
    Runs the specified file against a list of tags.

    Args:
        file (str): The file to be checked.
        tags (list): A list of tags to check the file against.
    ...
    for tag in tags:
        ...
        check_files(file, tag)


def cleartags(file):
    """
    Clears the tags of a file.

    Args:
        file (str): The path to the file to clear the tags of.
...
def update_tags(file, tag):
    """
    Update the tags of a file.

    Args:
        file (str): The path to the file to update the tags of.
